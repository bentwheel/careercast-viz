...
plotRanking <- function(metric, data, nthresh, mthresh, highlight=F)
{
  
  #metric <- "proj_growth_rank"
  #nthresh <- 8 
  #mthresh <- 3
  ...
  evalExtra <- "return(plot)"
  if(highlight) 
    evalExtra <- 'return(plot + gghighlight(career %in% c("Actuary","Data Scientist")))'
  ...
  plot.data <- data %>% 
    select(career, overall_rank, year) %>% 
    separate(overall_rank, into=c("num","denom"), sep="/") %>% 
    mutate(num = as.numeric(num)) %>% 
    group_by(career) %>% 
    summarize(n = mean(num),
              m = n()) %>% 
    filter(n <= nthresh & m > mthresh) %>% 
    ungroup() %>% 
    left_join(plot.data)
  ...
  plot <- plot.data %>% 
    ...
    scale_color_manual(values =c("#d4b9da","#c994c7","#df65b0","#e7298a","#ce1256","#980043","#67001f")) +
...
overall_rank <- plotRanking("overall_rank", data=data, 8, 3, F)
ggsave("./overall_rank.png", overall_rank, width=7, height=5)
ggplotly(overall_rank)

overall_rank_a_ds <- plotRanking("overall_rank", data=data, 8, 3, T)
ggsave("./overall_rank_a_ds.png", overall_rank_a_ds)

plotRanking("stress_rank", data=data, 8, 3,T)


plotRanking("proj_growth_rank", data=data, 8, 3,T)
plotRanking("work_environment_rank", data=data, 8, 3,T)
